...
class RegressionHead(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch,final_ch):
        ...
        self.final = nn.Conv3d(mid_ch+out_ch, final_ch, kernel_size=1, padding=0, bias=True)
...
def normoaliza_box(box_list):
    ...
    max_list = torch.max(box_list.reshape(box_list.shape[0], box_list.shape[1], -1), dim=2)[0]
    box_list = torch.clip(box_list, min=0)
    box_list = torch.div(box_list, max_list.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1))
    return box_list

class rmsf_model(nn.Module):
    
    def __init__(self, in_channels=1, out_channels_first_conv=32, n_classes=1):
        super().__init__()

        n1 = out_channels_first_conv
        filters = [n1, n1 * 2, n1 * 4, n1 * 8]  #  32   64    128    256  
        ...
        self.regression_head=RegressionHead(filters[0],filters[0],filters[0],n_classes)
        #-----------------------------------------------------------------------------------------
        

    def forward(self, x, sse=None, resid=None, resname=None, input_size=40, out_size=10):
        x=normoaliza_box(x) 
        ...
        if resid is not None:          
            ...
            if resid.shape[-1] != input_size:
                csize = (input_size - resid.shape[-1]) // 2
                x = x[:,:,csize:-csize,csize:-csize,csize:-csize]                                     
                x = normoaliza_box(x)
        ...
        if resname is not None:          
            ...
            if resname.shape[-1] != input_size:
                csize = (input_size-resname.shape[-1]) // 2
                x = x[:,:,csize:-csize,csize:-csize,csize:-csize]                                     
                x = normoaliza_box(x)
            x = torch.cat([x, resname], 1)   
